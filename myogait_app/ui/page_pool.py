...
def render(show_header: bool = True, mode: str = "single") -> None:
    ...
    *mode* selects which cross-condition material Analysis's guided scopes
    ...
    - ``"single"`` (default, "One group") -- everything: overview, condition
      comparison, accuracy. A bridge until it is rebuilt in Advanced
      (Phase 3 of the audit's action plan).
    ...
    - ``"accuracy"`` ("Accuracy vs C3D") -- the vs-Vicon precision material
      *only* (aggregate agreement, ICC validity/test-retest, bundle export).
      "accuracy" used to show the full page like every other mode, just
      reordered ("mode is emphasis, not a filter"); the audit (UX-02)
      flagged that as misleading for a scope named after one analysis, so
      it is now a real filter -- the one mode that is.
    """
    if show_header:
        page_header(
            "Cohort",
