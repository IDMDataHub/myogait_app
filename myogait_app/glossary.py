...
GROUPS: list[tuple[str, list[Entry]]] = [
    ...
    (
        ...
        [
            ...
            Entry("calibration_max_offset_deg (compute_angles option)", "angles", "Skips neutral calibration for a joint, with a warning, when the estimated offset exceeds this many degrees - the guard against a clip whose 'neutral' window actually caught mid-gait motion silently shifting the whole cycle (myogait 0.8.0+)."),
            ...
            Entry("canonicalize_angle_signs", "angles", "Enforces a flexion-positive sagittal convention independent of walking direction, verified against the knee-flexion peak - without it, two passes walked in opposite directions, or a video compared against its C3D reference, can disagree in sign (myogait 0.8.0+)."),
            Entry("pelvis_obliquity (compute_angles output)", "angles", "Frontal-plane pelvic obliquity (inter-hip line vs horizontal), not sagittal tilt despite the historical 'pelvis_tilt' key it is also still exposed under - the honest name, from myogait 0.8.0. Shown as a single once-per-frame trace on the Kinematics tab, not split by side."),
            Entry("compute_c3d_reference_angles", "experimental_vicon", "Recomputes a joint from the 3-D marker positions load_c3d kept, instead of the 2-D sagittal projection everything else uses. The projection is faithful for hip/knee (r >= 0.99 vs a Vicon 3-D reference) but collapses the ankle (r ~ 0.4, ROM halved) - the '3-D ankle reference for C3D sources' toggle, on by default and a no-op for any non-C3D source (myogait 0.8.0+)."),
    ...
    (
        ...
        [
            Entry("segment_cycles", "cycles", "Splits the walk into one cycle per heel-strike-to-next-same-side-heel-strike, time-normalizes each to 0-100%, and computes per-side mean +/- SD curves. min_confidence/min_coherence (myogait 0.8.1+) additionally reject a cycle whose mean landmark confidence or frame coherence falls below a threshold, counted in summary['n_rejected_quality'] - the Cycle segmentation quality-gate controls."),
    ...
    (
        "8. C3D & data sources",
        [
            Entry("load_c3d", "experimental_vicon", "Reads 3-D marker trajectories from a .c3d motion-capture file with ezc3d, projects them into the same 2-D sagittal pivot format a video extraction produces. Below myogait 0.8.0, normalized the antero-posterior and vertical axes independently - the app corrects the resulting aspect-ratio distortion by default on those versions only; 0.8.0 fixed it upstream (isotropic normalisation)."),
            Entry("detect_c3d_convention", "experimental_vicon", "Scores five registered marker-naming conventions (Plug-in Gait, ISB medial+lateral, Helen Hayes, BioCV, underscore variant, plus this app's own nature_multimodal addition) against a file's own labels, picking whichever resolves at least 4 of 6 required lower-limb landmarks - what the C3D tab's 'Auto-detect' now tries before falling back to this app's own alias-plus-keyword scan (myogait 0.7.0+)."),
